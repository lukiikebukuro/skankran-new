...
get_city_water_data = water_models['get_city_water_data']
...
class AquaBotQuery(db.Model):
    ...
    user_profile = db.Column(db.String(50), default='Ogólny')  # 🆕 NLP: Rodzic/Biohacker/Pacjent/Ogólny
...
def update_or_create_lead(data):
    ...
    if lead:
        # Update existing
        lead.last_visit = datetime.now(timezone.utc)
...
@app.route('/api/analytics/user-profiles', methods=['GET'])
@login_required
@admin_required
@limiter.exempt
def analytics_user_profiles():
    """🆕 NLP: Analityka profili użytkowników (Rodzic/Biohacker/Pacjent)"""
    try:
        # Grupuj zapytania po user_profile
        results = db.session.query(
        ...
        profiles_data = [
            {'profile': r.user_profile, 'count': r.count}
            for r in results
        ]
        
        # Dodaj sensory insights (zapytania ze słowami kluczowymi smaku/zapachu)
        sensory_keywords = ['gorzki', 'metaliczny', 'chlorowy', 'żelazny', 'rdzawy', 'cuchnąca', 'śmierdzi', 'brzydki']
        sensory_queries = db.session.query(AquaBotQuery).filter(
            db.or_(*[AquaBotQuery.query.ilike(f'%{kw}%') for kw in sensory_keywords])
        ).order_by(AquaBotQuery.id.desc()).limit(10).all()
        ...
        sensory_data = [
            {
                ...
                'city': q.city,
                'organization': q.organization,
                'timestamp': q.timestamp.strftime('%Y-%m-%d %H:%M') if q.timestamp else 'Unknown'
            }
            for q in sensory_queries
        ]
        
        # 🆕 SENSORY COMPLAINTS: Kategoryzacja skarg (Zapach/Smak/Wygląd)
        sensory_categories = {
        ...
        # Policzyć ile skarg per kategoria
        sensory_counts = {}
        sensory_detailed = []
        
        for category, keywords in sensory_categories.items():
            # Znajdź zapytania z tymi keywordami
            category_queries = db.session.query(AquaBotQuery).filter(
                db.or_(*[AquaBotQuery.query.ilike(f'%{kw}%') for kw in keywords])
            ).order_by(AquaBotQuery.id.desc()).all()
            
            sensory_counts[category] = len(category_queries)
            
            # Dodaj top 5 z tej kategorii do detailed list
            for q in category_queries[:5]:
                sensory_detailed.append({
                    'category': category,
                    'query': q.query[:100],
                    'city': q.city,
                    'timestamp': q.timestamp.strftime('%Y-%m-%d %H:%M') if q.timestamp else 'Unknown'
        ...
        # Sortuj detailed by timestamp (newest first)
        sensory_detailed.sort(key=lambda x: x['timestamp'], reverse=True)
        
        return jsonify({
            'profiles': profiles_data,
            'sensory_insights': sensory_data,  # Backward compatibility
            'sensory_counts': sensory_counts,   # Nowe: liczniki kategorii
            'sensory_alerts': sensory_detailed[:10]  # Nowe: top 10 alertów
        })
...
def analyze_query_topics(queries):
    ...
    keywords = {
        ...
        # 🆕 NLP: Sensory Tagging (z README)
        'Smak/Zapach': ['smak', 'zapach', 'cuchn', 'śmierd', 'brzyd', 'gorzki', 'metaliczny', 'chlorowy', 'żelazny', 'rdzawy', 'nieprzyjemny'],
        # 🆕 NLP: User Intent Indicators
        'Rodzic': ['dziecko', 'niemowlę', 'maluch', 'syn', 'córka', 'rodzic', 'mama', 'tata', 'butelka', 'karmienie'],
        'Biohacker': ['filtr', 'osmoza', 'minerały', 'ph optymalne', 'alkaliczna', 'jonizator', 'destylowana', 'reverse osmosis', 'tds'],
        'Pacjent': ['choroba', 'alergia', 'skóra', 'AZS', 'łuszczyca', 'nerki', 'ciśnienie', 'serce', 'lekarz', 'atopowe', 'crohn']
    }
...
def classify_user_profile(query):
    """
    🆕 NLP: Klasyfikuje użytkownika na podstawie zapytania.
    Zwraca: 'Rodzic', 'Biohacker', 'Pacjent', lub 'Ogólny'
    
    Business value:
    - Rodzic: Target dla filtrów dla dzieci, edukacja o azotanach
    - Biohacker: Target dla premium osmoza, jonizatory
    - Pacjent: Target dla medycznych rozwiązań, konsultacje
    """
    if not query:
        return 'Ogólny'
    ...
    # Słowa kluczowe per profil (priorytet: Pacjent > Rodzic > Biohacker)
    # 🆕 Rozszerzone o polską fleksję i synonimy
    profiles = {
        'Pacjent': [
            # Choroby skóry
            'choroba', 'choroby', 'chorobą', 'chory', 'chora',
            'alergia', 'alergii', 'alergik', 'uczulenie', 'uczulony',
            'skóra', 'skórę', 'skóry', 'cera', 'cerę',
            'azs', 'atopowe', 'atopow', 'łuszczyca', 'łuszczyc', 'egzema',
            'wyprysk', 'świąd', 'swędzenie', 'wysypka',
            # Choroby wewnętrzne
            'nerki', 'nerek', 'nerkowa', 'dializy', 'dializ',
            'ciśnienie', 'ciśnienia', 'nadciśnienie', 'hipotonia',
            'serce', 'serca', 'sercow', 'kardiolog',
            'crohn', 'choroba leśniowskiego', 'jelita', 'jelit', 'ibd', 'ibs',
            'tarczyca', 'tarczycy', 'hashimoto',
            # Onkologia
            'rak', 'raka', 'nowotwór', 'nowotworowy', 'onkolog', 'chemia',
            # Medycyna
            'lekarz', 'lekarza', 'lekarska', 'medyczn', 'szpital', 'klinika',
            'diagnoza', 'leczenie', 'terapia', 'recepta'
        ],
        'Rodzic': [
            # Dziecko - wszystkie formy
            'dziecko', 'dziecka', 'dziecku', 'dzieci', 'dzieciom', 'dziecię',
            # Niemowlę
            'niemowlę', 'niemowlak', 'niemowlęcia', 'niemowlęta', 'niemowlęc',
            'noworodek', 'noworodk', 'bobas', 'bobasa',
            # Maluch
            'maluch', 'malucha', 'maluchy', 'maluszek', 'maluszka',
            'szkrab', 'brzdąc', 'berbec',
            # Rodzina
            'synek', 'synka', 'syn', 'córka', 'córki', 'córeczka',
            'rodzic', 'rodziców', 'rodzice', 'mama', 'mamy', 'tata', 'taty',
            'ojciec', 'ojca', 'matka', 'matki',
            # Karmienie
            'butelka', 'butelki', 'smoczek', 'kaszka', 'mleko modyfikowane',
            'karmienie', 'karmić', 'odżywka', 'mleko dla dzieci',
            # Ciąża
            'ciąża', 'ciąży', 'ciężarna', 'w ciąży', 'brzuszek',
            # Wiek
            'roczne', 'roczny', 'miesięczne', 'latka', 'latek'
        ],
        'Biohacker': [
            # Filtracja
            'filtr', 'filtra', 'filtrów', 'filtracja', 'filtrować', 'filtruję',
            'osmoza', 'osmozy', 'osmotyczny', 'reverse osmosis', 'ro',
            # Minerały
            'minerały', 'minerałów', 'mineralizacja', 'remineralizacja',
            'wapń', 'magnez', 'potas', 'elektrolity',
            # pH/Alkaliczność
            'ph ', 'ph.', 'ph?', 'optymalne ph', 'kwaśne', 'zasadowe',
            'alkaliczna', 'alkaliczność', 'jonizator', 'jonizacja',
            # Techniczne
            'destylowana', 'destylator', 'tds', 'ppm',
            'optymalizacja', 'optymalizować', 'biohacking', 'biohacker',
            # Jakość
            'najlepsza woda', 'najczystsza', 'czysta woda', 'jakość wody'
        ],
        # 🆕 NOWE PERSONY BIZNESOWE
        'Techniczny': [
            # TDS i jednostki
            'tds', 'ppm', 'mikrony', 'mikronów', 'mikron',
            'membrana', 'membrany', 'membranę',
            # Ciśnienie i przepływ
            'ciśnienie', 'bar', 'przepływ', 'przepływu', 'l/min',
            'wydajność', 'wydajności',
            # Montaż
            'montaż', 'montażu', 'gwint', 'gwintu', 'złączka', 'przyłącze',
            'schemat', 'schematu', 'instrukcja', 'manual',
            # Techniczne
            'specyfikacja', 'parametry', 'dane techniczne', 'wymiary'
        ],
        'Smakosz': [
            # Kawa/Herbata
            'kawa', 'kawy', 'kawę', 'espresso', 'latte', 'cappuccino',
            'herbata', 'herbaty', 'herbatę',
            'ekspres', 'ekspresu', 'ciśnieniowy', 'kolba',
            # Smak/Zapach (skargi)
            'smak', 'smaku', 'smakiem', 'posmak', 'smakuje',
            'zapach', 'zapachu', 'pachnie', 'śmierdzi', 'cuchnąca', 'cuchn',
            'chlor', 'chlorem', 'chlorowy',
            # Osad
            'osad', 'osadu', 'kożuch', 'kamień', 'kamienia',
            'degustacja', 'aromaty'
        ],
        'Ekonomiczny': [
            # Cena
            'cena', 'ceny', 'cenę', 'kosztuje', 'koszt', 'kosztu',
            'tanio', 'tani', 'taniej', 'najtaniej', 'drogo', 'drogi',
            'promocja', 'promocji', 'rabat', 'zniżka',
            # Opłacalność
            'opłaca', 'opłaca się', 'opłacalne', 'zwrot', 'zwrotu',
            'oszczędność', 'oszczędzić', 'zaoszczędzić',
            'eksploatacja', 'eksploatacji', 'ile kosztuje litr',
            # Butelki
            'butelki', 'zgrzewka', 'zgrzewki', 'plastik', 'pet',
            'dźwigać', 'nosić', 'sklep'
        ],
        'EkoWarrior': [
            # Środowisko
            'środowisko', 'środowiska', 'ekologia', 'ekologiczne', 'eko',
            'planeta', 'planety', 'ziemia', 'klimat',
            # Odpady
            'odpady', 'odpadów', 'śmieci', 'śmiecenie',
            'zero waste', 'zerowaste', 'less waste',
            'recykling', 'recyklingu', 'segregacja',
            # Plastik
            'plastik', 'plastiku', 'mikroplastik', 'pet',
            'wielorazowe', 'wielorazowy', 'wielorazowa',
            'jednorazowe', 'jednorazówki',
            # Misja
            'ratować', 'chronić', 'zrównoważony', 'sustainable'
        ]
    }
    
    # Check w kolejności priorytetu
...
# ============================================
# 🏙️ SINGLE CITY WATER DATA API
# ============================================

_city_data_cache = {}

@app.route('/api/water-data/<city_name>', methods=['GET'])
@limiter.limit("100 per minute")
def api_water_data_city(city_name):
    """
    Endpoint zwracający dane o jakości wody dla pojedynczego miasta z PostgreSQL.
    Cache: 5 minut.
    ...
    try:
        ...
        if cache_key in _city_data_cache:
            cached = _city_data_cache[cache_key]
            if (now - cached['timestamp']) < 300:  # 5 min TTL
                return jsonify({
                    'success': True,
                    'city': cached['data']['name'],
                    'data': cached['data'],
                    'source': 'postgresql-cached',
        ...
        city_data = get_city_water_data(city_name)
        ...
        app.logger.error(f"[API ERROR] /api/water-data/{city_name} failed: {e}")
        return jsonify({
            'success': False,
            'error': str(e),
            'city': city_name
...
def handle_aquabot_query(data):
    ...
    try:
        ...
        # 🔒 SECURITY: Don't escape bot_response - it's already sanitized in aquabotBackend.py
        # We NEED the HTML tags (<span class="dot">) to render colored dots in admin modal
        bot_response_summary = bot_response_full[:1000] if bot_response_full else None
        
        # 🆕 NLP: Classify user profile
        user_profile = classify_user_profile(raw_query)
        ...
        if existing_query:
            ...
            query = AquaBotQuery(
                ...
                user_profile=user_profile  # 🆕 NLP: Save user profile
            )
