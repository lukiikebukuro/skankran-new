...
class VisitorTrackerSkankran {
    ...
    setupEventListeners() {
        ...
        window.addEventListener('beforeunload', () => {
            ...
            // 🔒 SECURITY FIX: Use fetch instead of sendBeacon (sendBeacon doesn't support Content-Type)
            // Send session_end event with proper headers
            fetch('/api/visitor-tracking', {
                ...
                console.warn('🛰️ SATELITA: session_end failed:', err);
            });
        });
    ...
    async handleAquaBotResponse(query, botResponse, queryId = null) {
        ...
        console.log('🛰️ SATELITA: handleAquaBotResponse called');
        console.log('  Socket exists:', !!this.socket);
        console.log('  Socket connected:', this.socket && this.socket.connected);
        console.log('  Query:', actualQuery);
        console.log('  Bot response:', botResponse ? botResponse.substring(0, 50) + '...' : 'No response');
        ...
        if (this.socket && this.socket.connected) {
            ...
            console.warn('🛰️ SATELITA: WebSocket NOT connected - query NOT sent to backend!');
            console.warn('  This means admin dashboard will NOT receive this query');
        }
    }
