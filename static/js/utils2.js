// City list cache to avoid repeated API calls
let _cityListCache = null;
let _cityListCacheTime = 0;
const CITY_LIST_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

async function getCityList() {
    // Check cache
    const now = Date.now();
    if (_cityListCache && (now - _cityListCacheTime) < CITY_LIST_CACHE_TTL) {
        return _cityListCache;
    }
    ...
    try {
        ...
        if (result.success && result.cities) {
            _cityListCache = result.cities.map(c => c.name);
            _cityListCacheTime = now;
            return _cityListCache;
        }
    } catch (error) {
        console.warn('⚠️ Failed to load city list from PostgreSQL:', error.message);
    }
...
export async function suggestCities(val, inputId = 'city') {
    try {
        ...
        if (val.length > 0 && cityNames.length > 0) {
